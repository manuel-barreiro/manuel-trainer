=====================================
EXERCISE LIBRARY
=====================================

Catálogo de ejercicios con el status de Manuel. El agente `manuel-trainer` lo usa al
armar sesiones y al rotar accesorios entre mesociclos. Marcá cada ejercicio:

-------------------------------------
INSTRUCTIONS
-------------------------------------

For each exercise, mark your status:
  [YES]  — can do, happy to include
  [SUB]  — can do but prefer a substitute
  [NO]   — injury, equipment, or preference — do not programme

Add notes where useful (e.g. "only with safety bar", "elbow pain on heavy sets")

-------------------------------------
LOWER BODY — QUAD DOMINANT
-------------------------------------

Barbell Back Squat        [ ]
Barbell Front Squat       [ ]
Safety Bar Squat          [ ]
Hack Squat (machine)      [ ]
Leg Press                 [ ]
Bulgarian Split Squat     [ ]
Lunges (barbell/DB)       [ ]
Step Ups                  [ ]
Goblet Squat              [ ]
Leg Extension (machine)   [YES] — en superset con Lateral Raise (Extra)

-------------------------------------
LOWER BODY — POSTERIOR CHAIN
-------------------------------------

Conventional Deadlift     [ ]
Sumo Deadlift             [ ]
Romanian Deadlift         [ ]
Stiff Leg Deadlift        [ ]
Hip Thrust (barbell)      [ ]
Hip Thrust (machine)      [ ]
Glute Bridge              [ ]
Leg Curl (lying)          [ ]
Leg Curl (seated)         [ ]
Nordic Curl               [ ]
Good Morning              [ ]
Cable Pull Through        [ ]

-------------------------------------
UPPER BODY — PUSH (HORIZONTAL)
-------------------------------------

Barbell Bench Press       [YES] — ejercicio principal del Extra
Dumbbell Bench Press      [ ]
Incline Barbell Press     [ ]
Incline Dumbbell Press    [ ]
Decline Press             [ ]
Machine Chest Press       [ ]
Cable Fly                 [ ]
Dumbbell Fly              [ ]
Pec Dec (machine)         [ ]
Deficit Push-Up           [YES] — ejercicio principal del Día A (añadido)

-------------------------------------
UPPER BODY — PUSH (VERTICAL)
-------------------------------------

Barbell OHP               [YES] — Press Militar, ejercicio principal del Día B
Dumbbell OHP              [ ]
Arnold Press              [ ]
Seated Machine Press      [ ]
Landmine Press            [ ]
Lateral Raise (DB)        [YES] — en superset con Leg Extension (Extra)
Lateral Raise (cable)     [ ]
Rear Delt Fly (DB)        [ ]
Rear Delt Fly (machine)   [ ]
Face Pull (cable)         [ ]

-------------------------------------
UPPER BODY — PULL (HORIZONTAL)
-------------------------------------

Barbell Bent Over Row     [SUB] — reemplazado por Pendlay Row en el Día B
Dumbbell Row              [ ]
Cable Row (seated)        [ ]
Machine Row               [ ]
Chest Supported Row       [ ]
Meadows Row               [ ]
Pendlay Row               [YES] — compuesto fijo de tracción horizontal, Día B

-------------------------------------
UPPER BODY — PULL (VERTICAL)
-------------------------------------

Pull Up (bodyweight)      [YES] — Dominadas Agarre Amplio 45°, Día A
Weighted Pull Up          [YES] — progresión futura de las dominadas
Chin Up                   [ ]
Lat Pulldown (bar)        [YES] — ejercicio principal del Extra
Lat Pulldown (neutral)    [ ]
Single Arm Pulldown       [ ]
Straight Arm Pulldown     [ ]

-------------------------------------
ARMS
-------------------------------------

Barbell Curl              [ ]
Dumbbell Curl             [YES] — en superset con Ab Wheel (Día A) y solo (Extra)
Incline Dumbbell Curl     [ ]
Cable Curl                [ ]
Hammer Curl               [ ]
Preacher Curl (machine)   [ ]
Close Grip Bench Press    [ ]
Tricep Pushdown (cable)   [YES] — Tricep Curl, ejercicio del Extra
Overhead Tricep Ext (cable)[ ]
Skull Crushers            [ ]
Dips (tricep)             [YES] — con lastre, en superset con Leg Raise (Día B)
Wrist Curl (DB)           [YES] — antebrazo, Extra (añadido)

-------------------------------------
CORE
-------------------------------------

Plank                     [ ]
Cable Crunch              [ ]
Hanging Leg Raise         [YES] — en superset con Dips (Día B)
Ab Wheel                  [YES] — en superset con Bicep Curl (Día A)
Pallof Press              [ ]
Landmine Rotation         [ ]

-------------------------------------
CARRIES
-------------------------------------

OH Single Carry (DB)      [YES] — Día A, 3×50" (añadido)
Suitcase Carry (DB)       [YES] — Día B, 3×50" (añadido)

Nota: los carries son time-based; Hevy NO permite registrar RPE en ejercicios por
tiempo. NUNCA falta el RPE acá — se autorregulan por peso y duración, no por RPE.

-------------------------------------
CALVES
-------------------------------------

Standing Calf Raise       [ ]
Seated Calf Raise         [ ]
Leg Press Calf Raise      [ ]

-------------------------------------
NOTES & PREFERENCES
-------------------------------------

Injuries / limitations:
Ninguna lesión activa.

Favourite exercises (prioritise these):
Bench Press, Dominadas, Press Militar, Barbell Row, Dips

Exercises to substitute:
N/A por ahora.

Other notes for your PT:
- Juega fútbol martes, jueves y sábado — tren inferior cubierto por fútbol.
- No programar sentadilla ni peso muerto salvo que Manuel lo pida explícitamente.
- El único trabajo de pierna en gym es Leg Extension en superset con Lateral Raise.
- Carries son parte fija de la programación — no sustituir.
- Entrena desde los 16 años (9 años de base), nivel advanced-intermediate.
- Toma 5g de creatina monohidrato diarios.

=====================================
END OF EXERCISE LIBRARY
=====================================